...
void setup(){
  ...
  cp5.addToggle("air_valve")
     ...
     ;
      ...
      // create a toggle for controlling the air valve
  cp5.addToggle("sine_curve_stream")
     .setPosition(230,520)
     .setSize(30,30)
     .plugTo(this)
     ;
...
void draw(){
  ...
  //sineCurve = map(sin(millis()), -1, 1, 0, 255);
  
  if (sine_curve_stream) {
    sineValue+=sineStep;
    sineCurve = map(sin(sineValue), -1, 1, min, max);
    dataStream.push("data-stream", sineCurve);
    if (device_stream) myPort.write(1+"c"+int(map(sineCurve, min, max, 0, 255))+"w");
    println(1+"c"+int(map(sineCurve, min, max, 0, 255))+"w");
  }
  
  else if (streamCounter+incrementStep < rawData.length-1) { 
